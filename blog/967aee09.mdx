...
对于一个常见的随机变量 $P(x;\theta)$，其中的 $x$ 是表示随机变量，$\theta$ 是该概率分布模型的模型参数。在不同的模型下有各自的模型参数，比如 [二项分布(This page was not published)](./404) $p$，[正态分布(This page was not published)](./404) 的 $\mu,\sigma$。
...
:::warning 为什么需要取对数呢？
[Log Derivative Trick(This page was not published)](./404)
