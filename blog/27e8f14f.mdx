...
$$
p(X_{b}\mid X_{a}) = \frac{p_{X}(X_{a}X_{b})}{p(X_{a})}
$$
...
$$
\frac{p_{X}(X_{a}X_{b})}{p(X_{a})} = \frac{c_{1}\exp \left( -\frac{1}{2} \left( x_{a}^T-\mu_{a}^T,x_{b}^T-\mu_{b}^T \right)\Sigma^{-1}\left( X_{a}-\mu_{a}, X_{b}-\mu_{b}\right)   \right)}{c_{2}\exp \left( -\frac{1}{2} \left( x_{a}^T-\mu_{a}^T\right)\Sigma_{aa}^{-1}\left( X_{a}-\mu_{a}\right)   \right)} 
$$
## 相关资料
