...
分解是 $\boldsymbol{x}_{t-1}\to x_{t}$ 的过程，这个过程中有很多数据对 $(\boldsymbol{x}_{t-1},\boldsymbol{x}_{t})$，这样<Highlight>重建</Highlight>自然就是从这些数据中学习一个 $\boldsymbol{x}_{t}\to \boldsymbol{x}_{t-1}$ 的模型。设该模型为 $\boldsymbol{\mu}(\boldsymbol{x}_{t})$，那么很容易想到学习方案就是最小化二者的欧式距离：
...
$$
:::bug 为什么这里需要添加噪声项，并且该噪声项相比于降噪项大的多？
