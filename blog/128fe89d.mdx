...
::: quote Why?
我们令了两个新的变量，并且将满足方程修改为了方程组的形式，但是为什么？？
左部为什么是 $\dot{\mathbf{x}}$，这有什么用？目的是什么？

下面是仅凭我的直觉猜测，可能并不是真实的原因：

`结论：`$\dot{\mathbf{x}}$ 是为了迭代，数值进行求解。

考虑 $\dot{\mathbf{x}}$，根据导数的定义，进行离散化：
...
\end{array}
$$
其中 $\mathbf{\Phi}=\mathbf{I}+T \mathbf{A} ; \quad \mathbf{G}=T \mathbf{B}$

所以我们便可以迭代求解其中的状态变量。
![Untitled.bmp](https://images.puqing.work/2023/09/17/6506a666db5ab.bmp)

另外，上述过程是对于导数的一阶近似，可能有其他方法迭代。
...
::: quote 定义 **时不变系统**
动态系统满足线性时不变条件，当它许用的输入轨迹 $\mathbf{u}(t)$ 自初始条件 $\mathbf{x}_{0}$ 开始，从初始时间 $t_{0}$ 后一定时间 $T\ge 0$ 之内，系统输出为 $\mathbf{y}(t)=\boldsymbol{\psi}\left(t, \mathbf{u}, \mathbf{x}_{0}, t_{0}\right)$，总可以用以下形式表示：

$$
\boldsymbol{\psi}\left(t, \mathbf{u}(t), \mathbf{x}_{0}, t_{0}\right)=\boldsymbol{\psi}\left(t+T, \mathbf{u}(t-T), \mathbf{x}_{0}, t_{0}+T\right)
$$
:::
这里这个公式还是挺不太明白的。下面画一张图就知道了
